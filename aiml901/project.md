...
- An **8-minute pitch video**, which should contain:
    ...
    - A live demonstration of your solution, which must include a use of n8n. Note that this can be a proof of concept, but the AI part of this must be able to run.
    - An evaluation process with a live demonstration.
...
- **A working n8n workflow and evaluation mechanism** that you presented in the video.
...
| Prototype            | You are able to showcase and share a working prototype. **This prototype should use n8n and an AI API** (but not necessarily only that). It should also have an **evaluation mechanism** that may or may not be implemented in n8n. Complexity is not needed to get a high grade, but it should show that you seriously thought about the approach and iterated on it. The fact that the user experience looks nice is a plus. The teaching team should be able to run the n8n part of your prototype. The prototype does not have to be a fully working version of your idea; think of it as a proof of concept, with a focus on the functionality and runnability of the AI portion. | 40%        |
...
| 3         | Keep iterating.                                                                                                                          |
| 4         | Work on your evaluation process, keep iterating on the project.                                                                          |
...
- **You can work with several people on the same project, with restrictions.** 
	...
	- In the video participant must present a different part of the prototype that they worked on themselves. Therefore, each participant should have a different n8n workflow/evaluation process.
...
### AI Agent
A good place to start is with the `AI Agent` node. You can copy the following code and paste it into an n8n workspace to get started:
```JSON
{
  "nodes": [
    {
...
```
It's important to note that the `Prompt (User Message)` will be data that you want to transform with AI, while the `System Message` is _how_ you want to transform this data. The data can take many forms and is not necessarily a chat message from a user. This could be emails, the metadata from a website, or more! 

Focus first on creating a great system prompt. This is the core of the project, and this alone can be a great project. Think about the structure of Kai; it's literally just a system prompt! **This node must be able to run in order to receive full credit.**

### Remainder of the Workflow
Beyond this, we ask that you
- Create the skeleton of a workflow around the agent
- Add some form of evaluation to your project
It is completely acceptable if there are errors in your workflow and it does not run besides the AI Agent node; this is mostly to understand the vision of the project. This can be more nodes in n8n, a website made using Lovable that you would want to eventually connect the n8n workflow to, or more. Remember, this class is only 5 weeks and this only needs to be a proof of concept.
